...
On Arch, I had to `yay -S words`, but other Linuxes (and Macs, I think) come with <nobr><code>/usr/share/dict/words</code></nobr> pre-installed. If not, you can get the word list [here](https://gist.githubusercontent.com/wchargin/8927565/raw/d9783627c731268fb2935a731a618aa8e95cf465/words) and update the path.
...
Instead of creating new attempts from scratch every time — picture throwing darts into the search space at random — we can "walk around" the search space, constantly moving from the current state to a "nearby" state in some sense. This idea is called [local search](https://en.wikipedia.org/wiki/Local_search_(optimization)).
