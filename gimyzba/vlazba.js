 ...
 * @param {boolean} all_letters
 * @param {boolean} deduplicate
 * @param {boolean} require_rafsi
 * @param {number} limit
 * @returns {string}
 */
export function generate_gismu(words_str, weights_str, shapes_str, all_letters, deduplicate, require_rafsi, limit) {
    let deferred5_0;
    let deferred5_1;
    try {
        const ptr0 = passStringToWasm0(words_str, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ptr1 = passStringToWasm0(weights_str, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len1 = WASM_VECTOR_LEN;
        const ptr2 = passStringToWasm0(shapes_str, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len2 = WASM_VECTOR_LEN;
        const ret = wasm.generate_gismu(ptr0, len0, ptr1, len1, ptr2, len2, all_letters, deduplicate, require_rafsi, limit);
        var ptr4 = ret[0];
        var len4 = ret[1];
        if (ret[3]) {
            ptr4 = 0; len4 = 0;
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('vlazba_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
